  ...
  const articles = [
    {
      ...
      coverImage: '', // Add the path or URL to the cover image here
      ...
      {filteredArticles.map((article) => (
        <div className={styles['article-container']} key={article.id}>
          ...
          {article.content.map((paragraph, index) => (
            <p key={index} className={styles['article-paragraph']}>{paragraph}</p>
          ))}
          <p className={styles['article-date']}>{article.dateCreated}</p>
        </div>
      ))}
    </div>
  );
